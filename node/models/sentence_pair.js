...
SentencePair.updateSentencePair = function (sentencePair, cb) {
    SentencePair.collection.updateOne({ _id: mongoose.Types.ObjectId(sentencePair._id) }, { $set: { accepted: sentencePair.accepted, viewed:  sentencePair.viewed} }, { upsert: false }, function (err, doc) {
        if (err) {
            LOG.error(err)
            cb(err, null)
        }
