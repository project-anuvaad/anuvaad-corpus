...
def download_docx():
    LOG.debug('download-docx: started')
    ...
    try:
        ...
        try:
            LOG.debug('download-docx: finding process from basename : ' + str(n_filename[0]))
            ...
            if translationProcess is not None:
                ...
                LOG.debug('download-docx: process found for basename with name = ' + str(data))
            ...
            LOG.debug('download-docx: error in finding process for basename : ' + str(n_filename))
...
def saveTranslateDocx():
    ...
    LOG.debug('uploadTranslateDocx: started at ' + str(start_time))
    ...
    LOG.debug('uploadTranslateDocx: ended at ' + str(getcurrenttime()) + 'total time elapsed : ' + str(
...
def translateDocx():
    start_time = int(round(time.time() * 1000))
    LOG.debug('translateDocx: started at ' + str(start_time))
    ...
    LOG.debug('model meta data' + model_meta_data)
    ...
    LOG.debug("translate-doxc : " + filename_to_processed)
    ...
    LOG.debug('translateDocx: number of nodes ' + str(len(nodes)) + ' and text are : ' + str(len(texts)))
    translationProcess = TranslationProcess.objects(basename=basename)
    ...
    LOG.debug('translateDocx: ended at ' + str(getcurrenttime()) + 'total time elapsed : ' + str(
        getcurrenttime() - start_time))
    return res.getres()
...
def get_sentence_word_count():
    start_time = int(round(time.time() * 1000))
    LOG.debug('get_sentence_word_count: started at ' + str(start_time))
    ...
    try:
        ...
        LOG.debug('get_sentence_word_count : Error while extracting docx, trying to convert it to docx from doc')
        try:
            LOG.debug('here === 1  ==  '+ filepath)
            docx_helper.convert_DOC_to_DOCX(filepath)
            LOG.debug('here === 2  ==  '+ filepath)
            xml_content = docx_helper.get_document_xml(filepath)
            LOG.debug('here === 3  ==  '+ str(xml_content))

            xmltree = docx_helper.get_xml_tree(xml_content)
            LOG.debug('get_sentence_word_count : doc to docx conversion successful')
        except Exception as e:
            ...
            LOG.debug('get_sentence_word_count: ended at ' + str(getcurrenttime()) + 'total time elapsed : ' + str(
                getcurrenttime() - start_time))
            return res.getres()
    nodes = []
    texts = []
    if xmltree is None:
        res = CustomResponse(Status.FAILURE.value, ' uploaded file is corrupt')
        LOG.debug('translate_docx_v2: ended at ' + str(getcurrenttime()) + 'total time elapsed : ' + str(
            getcurrenttime() - start_time))
        return res.getres()
    ...
    try:
        ...
        LOG.debug('translate_docx_v2 : not pre-processing document')
    word_count = 0
...
def translate_docx_v2():
    start_time = int(round(time.time() * 1000))
    LOG.debug('translate_docx_v2: started at ' + str(start_time))
    basename = str(int(time.time()))
    current_time = datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
    ...
    LOG.debug("translate_docx_v2 : file name " + filename_to_processed)
    ...
    try:
        ...
        LOG.debug('translate_docx_v2 : Error while extracting docx, trying to convert it to docx from doc')
        try:
            docx_helper.convert_DOC_to_DOCX(filepath)
            xml_content = docx_helper.get_document_xml(filepath)
            xmltree = docx_helper.get_xml_tree(xml_content)
            LOG.debug('translate_docx_v2 : doc to docx conversion successful')
        except Exception as e:
    ...
    LOG.debug('sending data to elasticsearch =='+str(doc_report))
    try:
        create_dashboard_report(doc_report, 'doc_report')
    except Exception as e:
        log.error('translate_docx_v2 : error occurred for report saving, error is = ' + str(e))

    LOG.debug('translate_docx_v2 : number of nodes = ' + str(len(nodes)) + ' and text are : ' + str(len(texts)))
    translationProcess = TranslationProcess.objects(basename=basename)
    translationProcess.update(set__eta=(TEXT_PROCESSING_TIME) * (len(texts) + get_pending_nodes()) / 25)
    ...
    try:
        ...
        for tp in translationProcess:
            ...
            try:
                ...
                LOG.debug('get_pending_nodes : Exception occured while counting nodes for basename = ' + tp[
        ...
        LOG.debug('get_pending_nodes : Exception occured : error is = ' + str(e))
        return 0
    ...
    for node in nodes:
        ...
        if text is not None and text.strip() is not '':
            ...
            if url_end_point == 'translate-anuvaad' and targetLang == 'en' and isEnglish(text):
                ...
                LOG.debug('send_nodes : Not sending particular node, because it already contains english, '
                ...
                LOG.debug('send_nodes : text in a node == ' + node.text)
                if not token_len == 0:
                    ...
                    for token in tokens:
                        if i == 25:
                            LOG.debug('send_nodes : in 25 final msg is == ' + str(msg))
                        ...
                        LOG.debug('send_nodes : message is = ' + str(msg))
                        messages.append(msg)
                        i = i + 1
                    msg = {'url_end_point': url_end_point, 'message': messages}
                    LOG.debug('send_nodes : final msg is == '+str(msg))
                    producer.send(TOPIC, value=msg)
                    producer.flush()
                    LOG.debug('send_nodes : flushed')
